/*
    ...
    단, 해당 데이터를 자식컴포넌트가 아니라 후손컴포넌트에서만 필요로 하는 데이터라면 자식컴포넌트에서는
    ...
    몇 번때 하위컴포넌트인지 상관없이 필요한 하위컴포넌트에서 소비자를 임포트해 필요한 데이터를 사용할 수 있음
...
class ContextApi extends React.Component {
    ...
    // 컨텍스트이용하여 자식컴포넌트에서 부모컴포넌트의 데이터를 변경하기
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        const content = {
            ...
            /* 
                자식에게 넘겨줄 데이터에 부모의 상태값과 상태값(state)을 변경해줄 수 있는 함수(setState)를 넘겨줌
            */
        }
