...
/*
    콜백함수? 
    자바스크립트의 함수 중 몇몇 코드는 비동기적으로 작동하는 코드는
    실행된 작업이 끝나지 않았더라도 다음 작업이 시작될 수 있는데, 이 때 특정 코드를 내가 원하는
    시점에 실행될 수 있도록 컨트롤해줘야할때 콜백함수를 사용한다.
    (일반함수 -> stack, 비동기함수 -> queue)

    Promise?
    콜백함수와 마찬가지로 비동기적으로 동작하는 코드를 동기적으로 구현할때 사용한다. 차이점은 콜백함수에 비해서
    가독성을 높일 수 있고, 예외처리를 쉽게 할 수 있다.
*/
...
class Callback extends Component {
    ...
    componentDidMount() {
        this.logPrint(1, function(return1){
            ...
            /*
                콜백함수를 이용하더라도 내가 원하는 실행 흐름대로 코드를 작성할 수는 있다.
                하지만 콜백함수를 여러번 사용할수록 코드가 더 지저분해진다는 단점도 존재함.
                이 때 대체재로 나온 기능이 Promise.
            */
        }.bind(this)
        /*
            화살표함수가 아닌 익명함수는 this가 window객체로 바인드되기 때문에 this가 현제 객체를 따라갈 수 있도록
            바인드메소드를 사용함
    ...
    logPrint(param, callback) {
        ...
        param += param; // 전달받은 param값게 다시 param값을 더해준 후
        ...
        */
        new Promise(
            resolve => {
                setTimeout(function() {
                    resolve('promise');
                    // 1.5초 후에 resolve함수가 실행되며 이 때 이행상태로 변경이 되면서
...
class PromiseCatch extends Component {
    ...
    componentDidMount() {
        /*
            reject? 대기, 이행, 거부 중 거부상태가 되었을 때 catch함수를 실행한다.
            작업도중 에러가 발생해 이행상태로 변화되지 못하는 경우 예외처리구문을 작성한다.

        */
        new Promise(
            (resolve, reject) => {
                // 첫 번째 파라미터는 무조건 이행함수, 두 번째 파라미터는 거부함수로 사용하면 된다.
                reject(Error("Error"));
                resolve("promise");
